...
//                    System.out.println("Opgehaald OrderID: " + rs.getInt("OrderID"));
    ...
    */
    //de route wordt niet HELEMAAL uit de database verwijderd
    public static boolean routeVerwijderen(int routeID){
        boolean isVerwijderd=false;

        //orderlijst ophalen van route
        ArrayList<Order> bestellingen = showRoute(routeID);
        connection= DatabaseConnectie.getConnection();

        SimpleDateFormat formatter= new SimpleDateFormat("yyyy-MM-dd");
        Date date = new Date(System.currentTimeMillis());//ophalen huidige datum

        //queries om een route op te slaan
        String updateRouteStatus= "UPDATE route SET Status='Anders', Opmerkingen='route verwijderd' WHERE RouteID=?";
        String lockTabel= "LOCK TABLE orders WRITE";//tabel opslot zetten
        String aanpassenOrder="UPDATE orders SET LastEditedWhen=?, Status='Anders', Comments='Bestelling uit route verwijderd' WHERE OrderID=?";
        String openTabel= "UNLOCK TABLES";
        ...
        try {
            ...
            //Route aanmaken
            PreparedStatement aanmakenRouteStmt = connection.prepareStatement(updateRouteStatus);
            aanmakenRouteStmt.setInt(1, routeID);
            aanmakenRouteStmt.executeUpdate();

            //orders tabel op slot zetten
            PreparedStatement tabelOpslotStmt = connection.prepareStatement(lockTabel);
            tabelOpslotStmt.executeUpdate();


            //per bestelling de Status,LastEditedWhen en Comments kolommen aanpassen
            for (Order bestelling: bestellingen) {
                PreparedStatement orderAanpassenStmt = connection.prepareStatement(aanpassenOrder);
                orderAanpassenStmt.setDate(1, Date.valueOf(formatter.format(date)));
                orderAanpassenStmt.setInt(2, bestelling.getOrderID());//get orderID
            ...
            System.out.println("Er is een fout: " + sqle.getSQLState());
            try {
                connection.rollback();//proberen om alles terug te draaien
            } catch (SQLException throwables) {
                throwables.printStackTrace();
